...
Supported commands: `/clear`, `/reset`, `/exit`, `/quit`, `/bye`, `/model <name>`
...
- Backends and models: Backends and model presets are defined in `src/agentc/providers.toml`; the repository default preset is `ollama-gpt-oss-120b` (see `core/config.py`), which targets the `ollama` backend using the `gpt-oss:120b-cloud` model string at `http://localhost:11434/v1`.
